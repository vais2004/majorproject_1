...
//import { addToCartAsync, removeFromCartAsync } from "../features/cartReducer"; // fixed imports here
import { addToWishlist, removeFromWishlist } from "../features/wishlistReducer";
...
export default function ProductDetails() {
  ...
  const handleToggleCart = (outfit) => {
    const isInCart = cartItems.some((item) => item._id === outfit._id);
    if (isInCart) {
      dispatch(removeFromCartAsync(outfit._id));  // fixed here
    } else {
      dispatch(addToCartAsync({ ...outfit, quantity: 1 }));  // fixed here
    }
  };
